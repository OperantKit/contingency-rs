...
DSL parsing is intentionally **Python-only**. The Rust runtime
accepts schedules constructed directly via `ScheduleBuilder`. There
is no Rust-side DSL bridge; consumers wishing to drive the Rust
runtime from a DSL source should parse with `contingency-py` and
transfer the resulting schedule across the FFI boundary.
...
| 2 | Conjunctive, Mixed, Overlay, Interpolate | `builder.rs` (`ScheduleBuilder` facade) |
| 3 | Concurrent `cod_directional` + component-form `punish` | `hw/` crate (`Apparatus` trait + `VirtualApparatus` + `SerialApparatus`); cross-binding E2E tests (`e2e_c_ffi.rs`, `e2e_wasm.rs`, `e2e_uniffi.rs`); `proptest` integration (`tests/properties.rs`); live `ffi.rs` / `wasm.rs` / `uniffi_api.rs` surfaces |
| 4 | Sidman, DiscriminatedAvoidance, Escape (aversive family); MatchingToSample, GoNoGo (trial-based family) | — |
...
| `src/contingency/builder.py` | `src/builder.rs` | `ScheduleBuilder` is a native Rust facade (27 factory methods). |
| `src/contingency/bridge/__init__.py` | *(no Rust counterpart — DSL parsing is Python-only)* | |
...
| `src/contingency/schedules/concurrent.py` | `src/schedules/concurrent.rs` | `Concurrent`, with `cod_directional: IndexMap<(String, String), f64>` and component-form `punish: IndexMap<String, Box<dyn Schedule>>`. |
...
files are binding glue or shared utilities. The `bridge/` module
(DSL parsing) is intentionally absent from the Rust port.
...
| `from_dsl` | N/A — DSL parsing is Python-only (see Gap G2). |
...
DSL parsing is implemented in Python only (`contingency.bridge.from_dsl`).
The Rust runtime intentionally has no DSL parser. To drive the Rust
runtime from a DSL source, the recommended path is:

1. Parse and resolve the DSL with `contingency-py` to obtain a
   `Schedule` instance.
2. Either:
   - run the schedule from Python and call into Rust components only
     for performance-critical kernels via PyO3, or
   - port the resolved structure to a `ScheduleBuilder` call sequence
     in Rust at code-generation time.

There is no Rust function exposed under `bridge::` and none is
planned.

### 2.4 `contingency.builder`

`ScheduleBuilder` is native in both ports. Rust signature: all
...
| InterlockingSchedule | `InterlockingSchedule` | *(none; covered via integration)* | `InterlockingSchedule` | `schedules/interlocking.rs` (8) |
| SecondOrder | `SecondOrder` | *(none)* | `SecondOrder` | `schedules/second_order.rs` (4) |
...
| Sidman | `Sidman` | `test_aversive.py` (33) | `schedules::aversive::tests` | Shared family coverage. |
...
types. Additional Rust coverage: `tests/properties.rs` (13 proptest
cases), `tests/conformance.rs` (14 strict + 6 ignored).
...
All four binding surfaces expose live symbols and ship E2E tests
that exercise the cross-language boundary.
...
| G2 | DSL bridge. **By design** — DSL parsing remains Python-only. The Rust runtime is driven via `ScheduleBuilder`; DSL consumers go through `contingency-py`. | `BY_DESIGN` |
...
| G9 | **Aversive and trial-based schedule families ported.** Rust `src/schedules/aversive.rs` (`Sidman`, `DiscriminatedAvoidance`, `Escape`) and `src/schedules/trial_based.rs` (`MatchingToSample`, `GoNoGo`) now exist. | Resolved |
...
schedule families have runtime support in Rust.

Verdict: **PASS** — three `BY_DESIGN` items remain (G2 DSL parsing
Python-only, G5 PRNG divergence, G7 error-variant flattening). Zero
functional gaps for `ScheduleBuilder`-driven workflows.
...
- Rust: 429 lib + 14 conformance (strict) + 1 e2e_c_ffi + 1 e2e_uniffi + 1 e2e_wasm + 7 ffi_smoke + 2 hil_integration + 12 hw_serial + 16 hw_virtual + 13 proptest + 1 python_smoke + 14 uniffi_smoke + 1 doc-test = **512 tests green**; 6 stochastic conformance tests `#[ignore]`'d by default.
